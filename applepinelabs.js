...
const appleState = {
    ...
    mappings: {} // Store mappings for each provider
};
...
const appleElements = {
    ...
    mappingTablesContainer: document.getElementById('appleMappingTablesContainer'),
...
const resetAppleInteractionState = () => {
    ...
    if (appleElements.mappingTablesContainer) {
        appleElements.mappingTablesContainer.innerHTML = '';
    }
...
const generateTableHtml = (provider, headers, rows, headerClass) => {
    const headerHtml = `<tr>${headers.map(h => `<th class="${headerClass || 'th-default'}">${h}</th>`).join('')}</tr>`;
    const bodyRowsHtml = rows.map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('');
    return `
        <h3 class="provider-table-title">${provider} Mapping</h3>
        <div class="overflow-x-auto">
            <table id="appleMappingTable-${provider}" class="apple-mapping-table">
...
const generateAppleMapping = () => {
    ...
    if (appleState.selectedTypes.includes('Finance') && appleState.selectedFinances.length) {
        appleState.selectedFinances.forEach(formatKey => {
            ...
            tablesHtml += generateTableHtml(formatKey, headers, rows, format.headerClass);
    ...
    if (appleState.selectedTypes.includes('Pinelabs')) {
        ...
        if (pinelabsData) {
            ...
            tablesHtml += generateTableHtml('Pinelabs', pinelabsData.headers, pinelabsData.rows, pinelabsData.headerClass);
    ...
    // Display all tables
    if (appleElements.mappingTablesContainer) {
        appleElements.mappingTablesContainer.innerHTML = tablesHtml;
    }
...
const exportAppleToExcel = () => {
    ...
    // Sort providers for consistent sheet order (Finance providers alphabetically, then Pinelabs)
