  ...
  mappingFormats: {
      Pinelabs: {
          headers: [
              ...
              'Email Address', 'RFSM', 'ASM', 'Bank Name', 'POS ID', 'TID',
          ...
          rowGenerator: (store, sheet2Data, contactInfo, requestDate) => {
              ...
              return matchingMachines.map(machine => {
                  const tidKeys = ['Plutus TID', 'PLUTUS TID', 'Plutus_TID', 'TID'];
                  const plutusTID = tidKeys.reduce((val, key) => val || machine[key], null) || '-';
                  return [
                      appleId, 'Non TEP', 'Redington', storeName,
                      address, city, pinCode, state,
                      contactInfo.person, contactInfo.number, contactInfo.email,
                      'Shankar Narayan', 'Vamsi', 'HDFC',
                      machine['POS ID'] || '-', plutusTID,
                      requestDate, appleId
                  ];
              });
          },
