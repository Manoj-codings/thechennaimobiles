const CONFIG = {
  ...
  mappingFormats: {
    BAJAJ: {
      headers: [
        ...
        "Retailer ID",
        "Dist ID",
        "Dealer type (Direct/Indirect)",
        "Dealer category (LFR/Regional Retailer/Brand store/Others)",
        "City",
        "State",
        "OEM Branch",
      ],
  ...
  emailDetails: {
    BAJAJ: {
      ...
      cc: [],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | Bajaj Finance - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
    ...
    HDB: {
      ...
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | HDB Finance - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    HDFC: {
      to: [
        "mayuresh.yadav@hdfcbank.com",
        "kushagra.chaturvedi@hdfcbank.com",
        "Hdfc.brandmapping@hdfcbank.com",
      ],
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | HDFC Finance - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    IDFC: {
      to: [
        "hemant.kumar_buz@idfcbank.com",
        "shivanshi.sharma@idfcfirstbank.com",
      ],
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | IDFC FIRST Bank - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    ICICI: {
      to: [
        "probir.bose@icicibank.com",
        "subhendu.panda@icicibank.com",
        "swaraj.baderao@icicibank.com",
      ],
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | ICICI Finance - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    TVS: {
      ...
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | TVS Finance - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    BENOW: {
      ...
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | Benow - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    KOTAK: {
      ...
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | Kotak Finance - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
    HMC: {
      ...
      cc: ["ravindra@xiaomi.com", "ashraf@xiaomi.com"],
      subject: (dateStr, storeLabel) =>
        `The Chennai Mobiles (TCM) Daily Mapping | HMC - ${dateStr} | ${storeLabel}`,
      body: (financeName, storeLabel) =>
        `Dear Team,\n\nPlease find the attached file containing the brand mapping details for ${financeName}, for the following store(s): ${storeLabel}.\n\n@ravindra@xiaomi.com Kindly review and approve the mapping at your earliest convenience.\n\nThank you for your support.`,
    },
  },
...
const appState = {
  ...
  miState: {
    // Added to store MI-specific state
    selectedStores: [],
    selectedType: "",
    selectedFinance: "",
    loadedFileName: "",
...
const inferState = (city) => {
  if (!city) return "-";
  return CONFIG.keralaCities.includes(city.toUpperCase().trim())
    ? "Kerala"
    : "TamilNadu";
};
...
const getStoreLabel = (stores) => {
  ...
  const shortNames = stores
    .map((store) => store.match(/^(\S+)/)?.[1] || store)
    .filter(Boolean);
  return shortNames.length ? shortNames.join(", ") : "Multiple Stores";
};

const getFinanceDisplayName = (financeKey) =>
  CONFIG.financeDisplayNames[financeKey] || financeKey || "Finance Provider";
...
const generateMapping = () => {
  ...
  if (
    !appState.storeData.length ||
    !appState.selectedStores.length ||
    !appState.selectedType
  ) {
    showError(
      "Please ensure a file is uploaded, a type is selected, and at least one store is selected."
    );
    return;
  }
  ...
  if (appState.selectedType === "Pinelabs") {
    if (typeof window.pinelabsGenerateMapping === "function") {
      window.pinelabsGenerateMapping(
        appState.storeData,
        appState.sheet2Data,
        appState.selectedStores
      );
  ...
  if (appState.selectedType === "Finance") {
    ...
    const stores = appState.storeData.filter((s) =>
      appState.selectedStores.includes(s.STORE)
    );
    ...
    const headerHtml = `<tr>${format.headers
      .map((h) => `<th class="${format.headerClass || "th-default"}">${h}</th>`)
      .join("")}</tr>`;
    const bodyRowsHtml = stores
      .map((store) => {
    ...
    if (emailDetails) {
      ...
      const dateStr = `${today.getDate().toString().padStart(2, "0")}_${(
        today.getMonth() + 1
      )
        .toString()
        .padStart(2, "0")}_${today.getFullYear()}`;
      elements.emailTo.textContent = emailDetails.to?.length
        ? emailDetails.to.join(", ")
        : "-";
      elements.emailCC.textContent = emailDetails.cc?.length
        ? emailDetails.cc.join(", ")
        : "-";
      elements.emailSubject.textContent = emailDetails.subject(
        dateStr,
        storeLabel
      );
      elements.emailBody.value = emailDetails.body(
        financeDisplayName,
        storeLabel
      );
...
const exportToExcel = () => {
  ...
  if (
    appState.selectedType === "Pinelabs" &&
    typeof window.pinelabsExportToExcel === "function"
  ) {
  ...
  if (appState.selectedType === "Finance" && appState.selectedFinance) {
    ...
    const headers = Array.from(elements.tableHead.querySelectorAll("th")).map(
      (th) => th.textContent.trim()
    );
    const rows = Array.from(elements.mappingBody.querySelectorAll("tr")).map(
      (row) =>
    ...
    for (let r = 0; r < wsData.length; r++) {
      for (let c = 0; c < headers.length; c++) {
        ...
        if (!ws[cellAddress])
          ws[cellAddress] = { t: "s", v: (wsData[r][c] || "").toString() };
    ...
    if (wsData.length > 1) {
      ws["!autofilter"] = {
        ref: XLSX.utils.encode_range(
          XLSX.utils.decode_range(ws["!ref"]).s,
          XLSX.utils.decode_range(ws["!ref"]).e
        ),
      };
    }
    ...
    const dateStr = `${today.getDate().toString().padStart(2, "0")}_${(
      ...
      .padStart(2, "0")}_${today.getFullYear()}`;
    const financeName =
      getFinanceDisplayName(appState.selectedFinance) || "Finance";
    const storeLabel = getStoreLabel(appState.selectedStores);
    const fileName = `TCM X MI Brand Mapping - ${dateStr} - ${storeLabel} - ${financeName}.xlsx`;
    XLSX.writeFile(wb, fileName);
  } else {
    alert(
      "Cannot export. Please ensure a Finance provider is selected and mapping is generated."
    );
  }
};
...
const copyEmailBody = async () => {
  ...
  if (
    appState.selectedType === "Pinelabs" &&
    typeof window.pinelabsCopyEmailBody === "function"
  ) {
...
const switchBrand = (brandId) => {
  ...
  document.querySelectorAll(".brand-section").forEach((section) => {
    section.classList.toggle(
      "hidden",
      section.id !== `brand-section-${brandId}`
    );
  });
...
const initializeEventListeners = () => {
  ...
  elements.typeSelect.addEventListener("change", (event) => {
    ...
    elements.financeSelectDiv.classList.toggle(
      "hidden",
      appState.selectedType !== "Finance"
    );
    elements.financeSelect.value = "";
...
const initializeApp = () => {
  ...
  const initialBrandId = initialActiveButton
    ? initialActiveButton.dataset.targetBrand
